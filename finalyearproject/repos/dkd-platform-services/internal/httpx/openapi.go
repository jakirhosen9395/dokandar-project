...
const openAPISpec = `{
  ...
  "info": {
    "title": "{{SERVICE}}",
    "version": "v1",
    "description": "DOKANDAR Context #13 — Platform notification-svc. R8 multi-channel citizen egress: enqueues SMS + USSD Bangla (<=160 char) notifications per citizen event; intra-context RabbitMQ with bounded-retry->park. Owns no business invariant (EF-OWN-4)."
  },
  "components": {
    "securitySchemes": {
      "Bearer": {
  ...
  "paths": {
    ...
    "/v1/notifications": {
      "post": {
        "operationId": "Enqueue",
        "summary": "Enqueue a citizen notification (SMS + USSD Bangla <=160 char pair); idempotent on Idempotency-Key",
        "parameters": [ { "name": "Idempotency-Key", "in": "header", "required": true, "schema": { "type": "string" } } ],
        "responses": { "201": { "description": "enqueued" }, "400": { "description": "validation error" } }
      },
      "get": {
        "operationId": "ListNotifications",
        "summary": "List notifications (cursor-paginated)",
        "parameters": [ { "name": "recipientDid", "in": "query", "required": false, "schema": { "type": "string" } } ],
        "responses": { "200": { "description": "notification page" } }
      }
    },
    "/v1/notifications/{ntf}": {
      "get": {
        "operationId": "GetNotification",
        "summary": "Fetch a single notification by NTF id",
        "parameters": [ { "name": "ntf", "in": "path", "required": true, "schema": { "type": "string" } } ],
