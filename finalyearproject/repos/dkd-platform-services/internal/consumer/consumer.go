...
const (
	// PLAT-06: a poison record is retried INLINE up to maxHandleAttempts (franz-go does not re-deliver
	// an uncommitted record within a session), then the park callback quarantines it to the DLQ.
	maxHandleAttempts  = 8
...
func (c *Consumer) Run(ctx context.Context) {
	for {
		...
		fetches.EachRecord(func(rec *kgo.Record) {
			...
			ev := toRawEvent(rec)
			// PLAT-06: bounded INLINE retry (transient failures recover), then park to the DLQ.
			var herr error
