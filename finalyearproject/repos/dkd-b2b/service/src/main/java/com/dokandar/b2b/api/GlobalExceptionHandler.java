 ...
 * Error codes follow dokandar.b2b.<category>.<reason>; internals never leak.
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Dto.Response<Void>> badRequest(Exception e) {
        return problem(400, Errors.errorCode("b2b", "request", "malformed"), e.getMessage());
    }

    @ExceptionHandler(NoResourceFoundException.class)
    public ResponseEntity<Dto.Response<Void>> noRoute(NoResourceFoundException e) {
        return problem(404, Errors.errorCode("b2b", "request", "not_found"), "no such resource");
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<Dto.Response<Void>> unexpected(Exception e) {
        log.error("unhandled error", e);
        return problem(500, Errors.errorCode("b2b", "internal", "unexpected"),
