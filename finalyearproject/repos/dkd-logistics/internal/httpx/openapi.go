...
const openAPISpec = `{
  ...
  "info": {
    "title": "{{SERVICE}}",
    "version": "v1",
    "description": "DOKANDAR Context #5 — Inventory & National Stock Ledger. READ-SIDE projection of custody (R1) + the G2 strong-LOCAL Reserve/Release seam. Reservation-critical reads never touch the eventual NIL rollup; NIL responses carry meta.asOf + lagMs (G11)."
  },
  "components": {
    "securitySchemes": {
      "Bearer": {
  ...
  "paths": {
    ...
    "/v1/inventory/reservations": {
      "post": {
        "operationId": "Reserve",
        "summary": "G2 strong-local reserve (row-locked CAS; idempotent on Idempotency-Key)",
        "parameters": [ { "name": "Idempotency-Key", "in": "header", "required": true, "schema": { "type": "string" } } ],
        "responses": { "201": { "description": "HELD reservation" }, "409": { "description": "INSUFFICIENT_STOCK" } }
      }
    },
    "/v1/inventory/reservations/{id}/release": {
      "post": {
        "operationId": "Release",
        "summary": "HELD -> RELEASED (idempotent compensating command)",
        "parameters": [ { "name": "id", "in": "path", "required": true, "schema": { "type": "string" } } ],
        "responses": { "200": { "description": "released" }, "409": { "description": "illegal transition" } }
      }
    },
    "/v1/inventory/reservations/{id}/confirm": {
      "post": {
        "operationId": "Confirm",
        "summary": "HELD -> CONSUMED",
        "parameters": [ { "name": "id", "in": "path", "required": true, "schema": { "type": "string" } } ],
        "responses": { "200": { "description": "consumed" }, "409": { "description": "illegal transition" } }
      }
    },
    "/v1/inventory/local": {
      "get": {
        "operationId": "GetLocalStock",
        "summary": "Strong-LOCAL stock position (reservation-critical reads land here, never NIL)",
        "parameters": [
          { "name": "gpid", "in": "query", "required": true, "schema": { "type": "string" } },
          { "name": "holder", "in": "query", "required": true, "schema": { "type": "string" } }
        ],
        "responses": { "200": { "description": "{onHand, held, available}" } }
      }
    },
    "/v1/inventory/nil/{gpid}": {
      "get": {
        "operationId": "GetNILRollup",
        "summary": "National stock rollup (eventual; meta.asOf + lagMs stamped — G11)",
        "parameters": [ { "name": "gpid", "in": "path", "required": true, "schema": { "type": "string" } } ],
        "responses": { "200": { "description": "rollup with locationBreakdown" } }
      }
    }
  }
}`
