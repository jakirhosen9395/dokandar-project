// Package outbox relays committed outbox rows to the Kafka spine (R6).
...
const MetricPublished = "logistics_events_published_total"
	...
	for _, row := range rows {
		rec := &kgo.Record{
			...
			Headers: []kgo.RecordHeader{
				...
				{Key: "producer_context", Value: []byte("logistics")},
			},
		}
