...
const openAPISpec = `{
  ...
  "info": {
    "title": "{{SERVICE}}",
    "version": "v1",
    "description": "DOKANDAR Context #13 Platform — append-only OHS audit sink (R6). The audit stream is ingested from the Kafka spine, not via HTTP; this surface exposes only the operational probes."
  },
  "components": {
    "securitySchemes": {
      "Bearer": {
