...
/** Injected upstream effects (real HTTP in index.ts; fakes in tests). */
export interface Deps {
  listProducts: () => Promise<Product[]>;
  resolveBuyer: (phone: string) => Promise<string>; // phone -> did (idempotent onboarding)
  placeOrder: (buyerDid: string, sellerDid: string, gpid: string, qty: number) => Promise<string>; // -> ORD
  getOrderStatus: (ord: string) => Promise<string | null>;
}

type Step = "MENU" | "PICK" | "QTY" | "CONFIRM" | "STATUS";
...
export interface UssdReply {
  cont: boolean; // true -> CON (keep open), false -> END
  message: string; // Bangla, <=160 chars
}

const con = (m: string): UssdReply => ({ cont: true, message: clip(m) });
const end = (m: string): UssdReply => ({ cont: false, message: clip(m) });

// USSD single-frame safety: hard-cap at 160 chars so a reply never overflows the GSM-7 frame.
function clip(m: string): string {
...
export function latestInput(text: string): string {
  if (!text) return "";
  ...
  switch (s.step) {
    case "MENU": {
      if (input === "1") {
        ...
        if (products.length === 0) return end("এই মুহূর্তে কোনো পণ্য নেই। পরে চেষ্টা করুন।");
        s.products = products;
        s.step = "PICK";
    ...
    case "PICK": {
      ...
      if (!p) return con("ভুল নম্বর। আবার বেছে নিন:\n" + numbered(s.products ?? []));
      s.gpid = p.gpid;
      s.seller = p.seller;
      s.step = "QTY";
      return con(`${p.name}\nপরিমাণ (কেজি) লিখুন:`);
    ...
    case "QTY": {
      ...
      if (!Number.isInteger(qty) || qty <= 0) return con("পরিমাণ একটি ধনাত্মক সংখ্যা হতে হবে। আবার লিখুন:");
      s.qty = qty;
      s.step = "CONFIRM";
    ...
    case "STATUS": {
      ...
      sessions.delete(sessionId);
      if (status === null) return end("অর্ডার পাওয়া যায়নি।");
...
function numbered(products: Product[]): string {
