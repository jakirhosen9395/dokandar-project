...
setInterval(() => { if (sessions.size > 10000)
    sessions.clear(); }, 300000).unref(); // crude TTL guard
async function httpJson(method, url, body, idem) {
...
const deps = {
    ...
    resolveBuyer: async (phone) => {
        // Idempotent onboarding: register the caller's phone; identity returns/keeps their DID.
        const r = await httpJson("POST", `${IDENTITY}/v1/parties`, { phoneNumber: phone, otpToken: "000000", locale: "bn-BD" }, `ussd-reg-${phone}`);
        const did = r.body?.data?.did ?? r.body?.did;
        if (!did)
            throw new Error(`identity did-resolve failed (${r.status})`);
        return String(did);
    },
    placeOrder: async (buyerDid, sellerDid, gpid, qty) => {
        ...
        if (!ord)
            throw new Error(`order failed (${r.status}: ${JSON.stringify(r.body?.error ?? r.body)})`);
        return String(ord);
    },
    getOrderStatus: async (ord) => {
...
async function handle(req, res) {
    ...
    if (req.method === "GET" && (path === "/health" || path === "/live")) {
        send(res, 200, { status: "ok", service: "ussd-ivr-bff", ...BUILD });
        return;
    }
    ...
    if (req.method === "GET" && path === "/version") {
        send(res, 200, { success: true, data: { service: "ussd-ivr-bff", ...BUILD }, error: null });
        return;
    }
    if (req.method === "POST" && path === "/ussd") {
        ...
        if (!sessionId || !phone) {
            send(res, 400, { type: "about:blank", status: 400, code: "dokandar.edge.ussd.bad_request", detail: "sessionId and phoneNumber are required" }, "application/problem+json");
            return;
        }
...
const server = createServer((req, res) => {
    handle(req, res).catch((e) => {
        if (!res.headersSent)
            send(res, 500, { type: "about:blank", status: 500, code: "dokandar.edge.internal.unexpected", detail: String(e) }, "application/problem+json");
    });
});
