...
import { composeOrderView, type Fetcher, type Upstreams } from "../src/view.js";

const UP: Upstreams = { b2c: "http://b2c", logistics: "http://log", finance: "http://fin",
  notification: "http://ntf" };

function fakeFetcher(routes: Record<string, { status: number; body: unknown }>): Fetcher {
  return async (url) => {
    for (const [key, out] of Object.entries(routes)) {
      if (url.includes(key)) return out;
    }
...
test("composes order + shipment + escrow + notifications", async () => {
  ...
  assert.equal((view["shipment"] as { shp: string }).shp, "SHP-1");
  assert.equal((view["escrow"] as { status: string }).status, "RELEASED");
  assert.equal((view["notifications"] as unknown[]).length, 1);
...
test("downstream outage degrades to warnings, never a failed page", async () => {
  ...
  assert.deepEqual(view["warnings"],
    ["shipment_unavailable", "escrow_unavailable", "notifications_unavailable"]);
});
