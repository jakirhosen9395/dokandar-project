...
import { replayBatch, isAllowed, type SyncOp, type Forwarder } from "../src/sync.js";

const okForward = (seen: SyncOp[]): Forwarder => async (op) => { seen.push(op); return { status: 201, body: { data: { ord: `ORD-${op.opId}` } } }; };
...
test("missing required fields → 400 per-op, batch continues", async () => {
  const results = await replayBatch(
    [{ opId: "x", idempotencyKey: "", method: "POST", path: "/v1/parties" } as SyncOp],
    okForward([]));
