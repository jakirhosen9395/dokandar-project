// Package config loads inventory-svc configuration from DKD_* environment variables.
// Inventory is a READ-SIDE projection of custody (R1) on the shared Postgres: dkd_inventory.
package config
...
func Load() Config {
	return Config{
		ServiceName:  getenv("DKD_SERVICE_NAME", "inventory-svc"),
		OtlpEndpoint: getenv("DKD_OTLP_ENDPOINT", ""),
		Context:     getenv("DKD_CONTEXT", "inventory"),
		...
		ConsumerGroup: getenv("DKD_CONSUMER_GROUP", "inventory-svc"),

		DBDSN:      os.Getenv("DKD_DB_DSN"),
		DBHost:     os.Getenv("DKD_DB_HOST"),
		DBPort:     getenv("DKD_DB_PORT", "5432"),
		DBName:     getenv("DKD_DB_NAME", "dkd_inventory"),
