...
wait_healthy() {
  printf '   waiting for healthy (scylla takes ~1 min)'
  for _ in $(seq 1 60); do
    [ "$(docker inspect -f '{{.State.Health.Status}}' dki_scylla 2>/dev/null || true)" = healthy ] && { echo ' ✓'; return 0; }
    printf '.'; sleep 3
  done
...
summary() {
  ...
  cat <<SUM
  ============ ScyllaDB ${SCYLLA_VERSION} (docker-single-node) ============
  CQL (local)   : 127.0.0.1:${SCYLLA_CQL_PORT}
  CQL (PUBLIC)  : ${PUBLIC_IP}:${SCYLLA_CQL_PORT}
  Auth          : none (learning utility)
  cqlsh (box)   : docker compose exec scylla cqlsh -e "SELECT release_version FROM system.local"
  Data (host)   : ${DATA_ROOT}/scylladb   (bind mount — survives 'down -v')
  ==============================================================
SUM
}
case "${1:-}" in
  ...
  purge)  load_env; docker compose down -v || true; sudo rm -rf "${DATA_ROOT}/scylladb"; ok "container + data deleted" ;;
