#!/usr/bin/env bash
# setup_env.sh — create/complete .env (idempotent). Generates RUSTFS_SECRET_KEY when empty.
set -euo pipefail
cd "$(dirname "$0")"
...
if grep -qE '^RUSTFS_SECRET_KEY=$' .env; then
  sed -i "s|^RUSTFS_SECRET_KEY=$|RUSTFS_SECRET_KEY=$(gen)|" .env; echo "generated RUSTFS_SECRET_KEY (24 chars)"
else echo "RUSTFS_SECRET_KEY already set — reused"; fi
chmod 600 .env
