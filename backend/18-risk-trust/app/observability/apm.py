...
# Default Python-agent processors (sanitizers) — PROCESSORS replaces the list, so we keep them.
_DEFAULT_PROCESSORS = (
...
def friendly_deps_processor(client, event):
    """Rename auto-instrumented HTTP dependency destinations to friendly store names so the
    Dependencies tab + service map show qdrant / scylladb instead of a raw IP:port (the
    qdrant-client + scylla driver expose no friendlier name)."""
    try:
        svc = event.get("context", {}).get("destination", {}).get("service", {})
        res = svc.get("resource", "")
        if isinstance(res, str):
            if res.endswith(":6333"):
                svc["resource"] = "qdrant"
            elif res.endswith(":9042"):
                svc["resource"] = "scylladb"
...
def drop_grpc_abort_errors(client, event):
    ...
    AbortError ('Locally aborted', culprit grpc._cython.cygrpc.abort) — the idiomatic gRPC status-return
    mechanism, an expected protocol OUTCOME, not a defect. Drop it from error capture (scoped to the
    exact AbortError class; every other exception, incl. real gRPC/qdrant faults, still surfaces)."""
    try:
