...
//   Mongo   — durable analytics (dokandar_logs.profile)
//   Elastic — Kibana Discover (logs-app-profile-*)
...
func Setup(ctx context.Context, level string, cfg SinkConfig) (*LogSinks, error) {
	...
	// Mongo — uses the same auth.dokandar_logs collection convention.
	if cfg.MongoURI != "" {
	...
	// Elasticsearch — bulk-POST via plain net/http.
	if cfg.ESURL != "" {
...
// MongoHealth — used by /ready and /health.
...
// emit in the auth service so Kibana/Mongo queries portable between Python
