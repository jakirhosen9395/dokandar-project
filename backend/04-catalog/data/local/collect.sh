#!/usr/bin/env bash
# data/local/collect.sh — writes data/local/result.json (served by GET /data when
# the service runs as TENANT=local). Collects the local-machine baseline. The
# data/ dir is bind-mounted read-only, so re-running refreshes /data with no restart.
...
read_os_pretty_name() {
  if [ -f /etc/os-release ]; then . /etc/os-release; printf '%s' "${PRETTY_NAME:-${ID:-unknown}}"
  elif [ "$(uname)" = "Darwin" ]; then printf 'macOS %s' "$(sw_vers -productVersion 2>/dev/null || echo unknown)"
  else printf 'unknown'; fi
}
json_escape() { local s="${1-}"; s="${s//\\/\\\\}"; s="${s//\"/\\\"}"; s="${s//$'\t'/\\t}"; s="${s//$'\r'/\\r}"; s="${s//$'\n'/\\n}"; printf '%s' "$s"; }
safe() { local v="${1-}"; if [ -z "$v" ]; then printf '%s' "${2:-unavailable}"; else printf '%s' "$v"; fi; }

HOSTNAME_VAL="$(hostname 2>/dev/null || echo unknown)"; OS_PRETTY="$(read_os_pretty_name)"
KERNEL="$(uname -r 2>/dev/null || echo unknown)"; ARCH="$(uname -m 2>/dev/null || echo unknown)"
...
CPU_LOAD="$(uptime 2>/dev/null | awk -F'load average:' '{gsub(/^ +/, "", $2); print $2}')"; CPU_LOAD="$(safe "$CPU_LOAD" "unknown")"
MEM_TOTAL="$(free -h 2>/dev/null | awk '/Mem:/ {print $2}')"; MEM_USED="$(free -h 2>/dev/null | awk '/Mem:/ {print $3}')"; MEM_AVAILABLE="$(free -h 2>/dev/null | awk '/Mem:/ {print $7}')"
MEM_TOTAL="$(safe "$MEM_TOTAL" unknown)"; MEM_USED="$(safe "$MEM_USED" unknown)"; MEM_AVAILABLE="$(safe "$MEM_AVAILABLE" unknown)"
DISK_TOTAL="$(df -h / 2>/dev/null | awk 'NR==2 {print $2}')"; DISK_USED="$(df -h / 2>/dev/null | awk 'NR==2 {print $3}')"; DISK_FREE="$(df -h / 2>/dev/null | awk 'NR==2 {print $4}')"; DISK_PCT="$(df -h / 2>/dev/null | awk 'NR==2 {print $5}')"
DISK_TOTAL="$(safe "$DISK_TOTAL" unknown)"; DISK_USED="$(safe "$DISK_USED" unknown)"; DISK_FREE="$(safe "$DISK_FREE" unknown)"; DISK_PCT="$(safe "$DISK_PCT" unknown)"
