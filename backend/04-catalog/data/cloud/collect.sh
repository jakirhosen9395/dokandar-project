#!/usr/bin/env bash
# data/cloud/collect.sh — writes data/cloud/result.json (served by GET /data when
# the service runs as TENANT=cloud). Collects the EC2 instance metadata (IMDSv2)
# plus the machine baseline. The data/ dir is bind-mounted RO, so re-running
# refreshes /data with no container restart.   ./data/cloud/collect.sh
set -euo pipefail
...
CPU_CORES="$(nproc 2>/dev/null || echo unknown)"; CPU_LOAD="$(uptime 2>/dev/null | awk -F'load average:' '{gsub(/^ +/, "", $2); print $2}')"; CPU_LOAD="$(safe "$CPU_LOAD" unknown)"
...
AVAILABILITY_ZONE="$(safe "$(meta placement/availability-zone)" unavailable)"; REGION="$(safe "$(dyn instance-identity/document | awk -F'\"' '/region/ {print $4}')" unavailable)"
