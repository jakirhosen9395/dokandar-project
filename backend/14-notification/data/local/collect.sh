#!/usr/bin/env bash
# data/local/collect.sh — writes data/local/result.json (served by GET /data when TENANT=local).
# 14-notification (Node 24 / Fastify 5) local-machine baseline + a static service-shape block.
