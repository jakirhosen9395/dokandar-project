...
// Ported verbatim from dokandar-profile; the labels and service.node.name
// semantics are identical across the Go fleet.
...
// Must run BEFORE the APM HTTP middleware so the tracer sees the labels on
// every transaction.
