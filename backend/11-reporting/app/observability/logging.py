...
class PrettyJsonFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        out: dict[str, Any] = {
            "asctime": datetime.fromtimestamp(record.created).strftime("%Y-%m-%d %H:%M:%S,") + f"{int(record.msecs):03d}",
            "name": record.name,
        ...
        try:
            ...
            txid = elasticapm.get_transaction_id()
        except Exception:  # noqa: BLE001
            tid = txid = None
        if tid:
            out["elasticapm_trace_id"] = tid
            out["elasticapm_transaction_id"] = txid
            out["elasticapm_service_name"] = settings.apm_service_name
        if record.exc_info:
...
def _record_to_doc(record: logging.LogRecord) -> dict:
    doc: dict[str, Any] = {
        "@timestamp": datetime.fromtimestamp(record.created, timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.") + f"{int(record.msecs):03d}Z",
    ...
    if tid:
        doc["trace"] = {"id": tid}
        doc["transaction"] = {"id": txid}
        doc["elasticapm_trace_id"] = tid
        doc["elasticapm_transaction_id"] = txid
        doc["elasticapm_service_name"] = settings.apm_service_name
    if record.exc_info:
...
async def _drain_es() -> None:
    ...
    while True:
        try:
            ...
            body = ("\n".join(lines) + "\n").encode("utf-8")
            try:
                r = await _es_client.post(bulk_url, content=body,
                                          headers={"content-type": "application/x-ndjson"})
                if r.status_code >= 300:
                    print(f"[es-sink] HTTP {r.status_code}: {r.text[:160]}", file=sys.stderr, flush=True)
            except Exception:  # noqa: BLE001
                pass
        except asyncio.CancelledError:
            return
