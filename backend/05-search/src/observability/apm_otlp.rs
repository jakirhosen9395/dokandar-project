...
//! Each HTTP request is a SERVER transaction; PG/ES/Kafka calls become CLIENT exit
//! spans nested under it (drives Dependencies + Service Map, like auth's agent spans).
...
pub fn init(cfg: &Config) {
    ...
    let resource = Resource::new(vec![
        ...
        KeyValue::new("service.framework.name", "axum"),
    ]);
...
pub fn server_context(method: &str, route: &str, path: &str) -> Option<Context> {
    if !on() { return None; }
    let tracer = global::tracer("dokandar-search");
...
/// A dependency (exit) span — `system` (postgresql/elasticsearch/kafka) drives the
...
/// A messaging consume transaction for a projector (drives Service-Map kafka edge).
pub fn consume_context(topic: &str) -> Option<Context> {
    if !on() { return None; }
    let tracer = global::tracer("dokandar-search");
    let mut span = tracer.span_builder(format!("{} receive", topic)).with_kind(SpanKind::Consumer).start(&tracer);
    span.set_attribute(KeyValue::new("messaging.system", "kafka".to_string()));
    span.set_attribute(KeyValue::new("messaging.destination.name", topic.to_string()));
    Some(Context::current_with_span(span))
}
...
fn init_metrics(cfg: &Config) {
    ...
    let meter = global::meter("dokandar-search");
