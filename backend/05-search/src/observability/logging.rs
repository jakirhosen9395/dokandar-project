...
fn ident() -> (&'static str, &'static str, &'static str, &'static str) {
    IDENT.get().map(|(n, v, e, h)| (n.as_str(), v.as_str(), e.as_str(), h.as_str())).unwrap_or(("05-search", "", "dev", ""))
}
...
pub fn emit(level: &str, logger: &str, message: &str, extra: Value) { emit_inner(level, logger, message, extra, apm_otlp::current_ids()); }
