 ...
 * — {@code /ready}, {@code /metrics}, AND {@code /health} — is excluded. The line
 * carries the TEMPLATED route (never the raw path — a raw {@code /orders/<uuid>}
 * would leak ids + blow up cardinality), the true client IP (X-Forwarded-For /
 * CF-Connecting-IP), status, latency_ms, request_id, trace_id.
 */
@Component
...
public class AccessLogFilter extends OncePerRequestFilter {
    ...
    /** Spring's best-matching pattern (e.g. /api/v1/order/orders/{id}); falls back to a sentinel, never the raw path. */
