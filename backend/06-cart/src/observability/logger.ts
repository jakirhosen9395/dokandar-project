// Three-sink structured logging mirroring 01-auth: stdout (JSON w/ elasticapm_*),
// MongoDB forensic sink, Elasticsearch data stream logs-app-06-cart-*. Plus a
// plain HTTP access-log line. Trace correlation via the Elastic APM Node agent.
import apm from '../apm';
...
export function log(level: string, name: string, message: string, extra: Json = {}): void {
  ...
  // ECS doc for the Mongo + ES sinks
  const doc: Json = {
