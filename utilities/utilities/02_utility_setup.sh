#!/usr/bin/env bash
#==============================================================================
# DOKANDAR utilities — SERVER A (utility2) fleet orchestrator
#------------------------------------------------------------------------------
# Brings up EVERY Server-B tool in one chosen install variant, then prints a
...
#
#   Usage:   bash 02_utility_setup.sh <variant> [action]
#
#     <variant>  01_native_single | 02_native_cluster | 03_docker_single | 04_docker_cluster
#     [action]   up (default) | down | purge | status
#
#   Examples:
#     bash 02_utility_setup.sh 03_docker_single          # bring the fleet up (Docker, single-node)  <-- recommended
#     bash 02_utility_setup.sh 01_native_single          # bring the fleet up (native pkgs, uses sudo)
#     bash 02_utility_setup.sh 03_docker_single status   # show each tool's status
#     bash 02_utility_setup.sh 03_docker_single down      # stop containers   (DATA PRESERVED)
#     bash 02_utility_setup.sh 03_docker_single purge     # stop + DELETE data
#
#   Server B tools: MongoDB · Elasticsearch · Kafka · RabbitMQ · OpenBao · RustFS · Qdrant · Neo4j · NATS · Temporal
#   (OpenBao lives here — its default port 8200 collided with Elastic APM on Server A, so it was moved here.)
#
#   Notes:
...
#     * Bundle is auto-located: $UTIL_BUNDLE, else ./utility2, else this dir itself.
#
...
#   UTIL_SKIP="" bash 02_utility_setup.sh 03_docker_single   # run the FULL fleet (skip nothing)
...
#     UTIL_SKIP="elastic_apm prometheus openbao" bash 02_utility_setup.sh 03_docker_single
